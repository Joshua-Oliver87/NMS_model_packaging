...
const PlantNConcentrationChart = ({ closeModal, chartData }) => {
  ...
  const handleDownload = (format) => {
    ...
    img.onload = () => {
      ...
      link.download = `plant-n-concentration.${format}`;
  ...
  useEffect(() => {
    setChartOptions({
      ...
      yAxis: {
        title: { text: "Crop N Conc (kg/kg)", style: { color: "black" } },
        labels: { style: { color: "crimson" } },
        min: 0,
      },
      series: [
        {
          name: "Crop N Conc (kg/kg)",
          data: Object.values(chartData)?.map((data) => {
            const value = data["Crop N Conc (kg/kg)"];
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center" }}>
          <span>Plant N Concentration</span>
          <IconButton
            aria-label="download"
...
export default PlantNConcentrationChart;